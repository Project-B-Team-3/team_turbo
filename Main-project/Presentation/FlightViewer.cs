...
public static class ConsoleView
{
	public static void DisplayFlights()
	{
		...
		{
			foreach (var flight in BookingLogic.UpComingFlights()) Console.WriteLine(flight);
	...
	public static void SelectFlight()
	{
		...
		for (var i = 0; i < flights.Count(); i++) Console.WriteLine($"{i + 1}: {flights.ElementAt(i).FlightNumber}");
		...
		{
			var flight = FlightDataAccess.GetFlights()
				.FirstOrDefault(f => f.FlightNumber == flights.ElementAt(index - 1).FlightNumber);
			if (flight != null)
