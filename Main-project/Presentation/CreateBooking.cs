	...
	public static void CreateNewBooking()
	{
		...
		if (answer == "y")
		{
			...
			for (var i = 0; i < cateringList.Count; i++)
				Console.WriteLine(
					$"{i + 1}. {cateringList[i].Name} ({cateringList[i].Price:C2}) - {cateringList[i].Description}");
			...
			foreach (var cateringItem in cateringItems) Console.WriteLine($"{cateringItem.Name}, ");
