...
public static class CreateBooking
{
    public static void CreateNewBooking()
    {
        ...
        Console.Clear();
        Console.WriteLine(
            "Welcome to the booking menu, please enter a flight number to book a flight."
        );
        ...
        if (
            ...
            || flightNum == ""
            || peopleCount == ""
            || !int.TryParse(peopleCount, out peopleInt)
        )
        {
        ...
        cost.FlightPrice = FlightDataAccess
            .GetFlights()
        ...
        if (answer == "y")
        {
            ...
            do
            {
                invalidInput = false;
                Console.WriteLine(
                    "Enter the number of the catering item you want to order (or '0' to exit):"
                );
